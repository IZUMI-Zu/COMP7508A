...
class Database:
    def __init__(
        self, motions, feature_mapping, selected_feature_names, selected_feature_weights
    ) -> None:
        self.dt = 1 / 60
        ...
        self.joint_name, self.joint_parent, self.joint_offset = (
            motions[0].joint_name,
            motions[0].joint_parent,
            motions[0].joint_offset,
        )
        self.joint_name = ["Simulation_Bone"] + self.joint_name
        self.joint_parent = [-1] + [i + 1 for i in self.joint_parent]
        self.joint_offset = np.concatenate(
            [[np.zeros((1, 3))], self.joint_offset], axis=0
        )[:, 0]
        self.joint_channel = np.array(
            [3 if "end" not in name else 0 for name in self.joint_name]
        )
        ...
        for bvh_item in motions:
            ...
            true_motion_data = np.zeros(
                (motion_data.shape[0], self.joint_channel_offset[-1])
            )
            joint_channel = np.array(
                [3 if "end" not in name else 0 for name in joint_name]
            )
            joint_channel[0] = 6
            ...
            for idx, name in enumerate(joint_name):
                ...
                true_motion_data[
                    :,
                    self.joint_channel_offset[true_idx] : self.joint_channel_offset[
                        true_idx + 1
                    ],
                ] = motion_data[
                    :, joint_channel_offset[idx] : joint_channel_offset[idx + 1]
                ]
            ...
            tmp_pos, tmp_rot = forward_kinematics_with_channel(
                self.joint_parent,
                self.joint_channel,
                self.joint_offset,
                true_motion_data,
            ...
            fwd_direction = np.array([1, 0, 1]) * sim_rotation.apply(
                np.array([0, 0, 1])
            )
            fwd_direction = fwd_direction / np.linalg.norm(
                fwd_direction, axis=1, keepdims=True
            )

            filtered_sim_position = signal.savgol_filter(
                sim_position, 31, 3, axis=0, mode="interp"
            )
            filtered_fwd_direction = signal.savgol_filter(
                fwd_direction, 61, 3, axis=0, mode="interp"
            )
            filtered_fwd_direction = filtered_fwd_direction / np.linalg.norm(
                filtered_fwd_direction, axis=1, keepdims=True
            )

            angle = np.arctan2(
                filtered_fwd_direction[:, 0], filtered_fwd_direction[:, 2]
            )
            filtered_sim_rotation = R.from_rotvec(
                np.array([0, 1, 0]).reshape(1, 3) * angle[:, np.newaxis]
            )
            true_motion_data[:, :3] = filtered_sim_position
            true_motion_data[:, 3:6] = filtered_sim_rotation.as_euler(
                "XYZ", degrees=True
            )
            true_motion_data[:, 6:9] -= filtered_sim_position
            true_motion_data[:, 9:12] = (
                filtered_sim_rotation.inv()
                * R.from_euler("XYZ", true_motion_data[:, 9:12], degrees=True)
            ).as_euler("XYZ", degrees=True)

            self.motion_data.append(true_motion_data)

            # Extract the data terms
            pos, rot = forward_kinematics_with_channel(
                self.joint_parent,
                self.joint_channel,
                self.joint_offset,
                true_motion_data,
            ...
            local_rot = (
                R.from_euler("XYZ", true_motion_data.reshape(-1, 3), degrees=True)
                .as_quat()
                .reshape(true_motion_data.shape[0], -1, 4)
            )
        ...
        self.frame_num, self.joint_num = (
            self.motion_data.shape[0],
            self.local_a_velocities.shape[1] // 3,
        )

        self.features_mean, self.features_std = (
            np.mean(self.features, axis=0),
            np.std(self.features, axis=0),
        )
    ...
    def extract_features(self, root_pos, root_rot, pos, rot, vel, avel):
        ...
        for feature_name in self.feature_names:
            if feature_name == "lFootPos":
                features.append(
                    self.extract_offset(
                        root_pos, root_rot, pos[:, self.joint_name.index("lToeJoint")]
                    )
                )
            elif feature_name == "rFootPos":
                features.append(
                    self.extract_offset(
                        root_pos, root_rot, pos[:, self.joint_name.index("rToeJoint")]
                    )
                )
            elif feature_name == "lFootRot":
                features.append(
                    self.extract_rotation(
                        root_rot, rot[:, self.joint_name.index("lToeJoint")]
                    )
                )
            elif feature_name == "rFootRot":
                features.append(
                    self.extract_rotation(
                        root_rot, rot[:, self.joint_name.index("rToeJoint")]
                    )
                )
            elif feature_name == "lFootVel":
                features.append(
                    self.extract_vel(
                        root_rot, vel[:, self.joint_name.index("lToeJoint")]
                    )
                )
            elif feature_name == "rFootVel":
                features.append(
                    self.extract_vel(
                        root_rot, vel[:, self.joint_name.index("rToeJoint")]
                    )
                )
            elif feature_name == "lHandPos":
                features.append(
                    self.extract_offset(
                        root_pos, root_rot, pos[:, self.joint_name.index("lWrist")]
                    )
                )
            elif feature_name == "rHandPos":
                features.append(
                    self.extract_offset(
                        root_pos, root_rot, pos[:, self.joint_name.index("rWrist")]
                    )
                )
            elif feature_name == "lHandRot":
                features.append(
                    self.extract_rotation(
                        root_rot, rot[:, self.joint_name.index("lWrist")]
                    )
                )
            elif feature_name == "rHandRot":
                features.append(
                    self.extract_rotation(
                        root_rot, rot[:, self.joint_name.index("rWrist")]
                    )
                )
            elif feature_name == "lHandVel":
                features.append(
                    self.extract_vel(root_rot, vel[:, self.joint_name.index("lWrist")])
                )
            elif feature_name == "rHandVel":
                features.append(
                    self.extract_vel(root_rot, vel[:, self.joint_name.index("rWrist")])
                )
            elif feature_name == "rHipPos":
                features.append(
                    self.extract_offset(
                        root_pos, root_rot, pos[:, self.joint_name.index("lHip")]
                    )
                )
            elif feature_name == "lHipPos":
                features.append(
                    self.extract_offset(
                        root_pos, root_rot, pos[:, self.joint_name.index("rHip")]
                    )
                )
            elif feature_name == "rHipRot":
                features.append(
                    self.extract_rotation(
                        root_rot, rot[:, self.joint_name.index("lHip")]
                    )
                )
            elif feature_name == "lHipRot":
                features.append(
                    self.extract_rotation(
                        root_rot, rot[:, self.joint_name.index("rHip")]
                    )
                )
            elif feature_name == "rHipVel":
                features.append(
                    self.extract_vel(root_rot, vel[:, self.joint_name.index("lHip")])
                )
            elif feature_name == "lHipVel":
                features.append(
                    self.extract_vel(root_rot, vel[:, self.joint_name.index("rHip")])
                )
            elif feature_name == "hipPos":
                features.append(
                    self.extract_offset(
                        root_pos, root_rot, pos[:, self.joint_name.index("RootJoint")]
                    )
                )
            elif feature_name == "hipRot":
                features.append(
                    self.extract_rotation(
                        root_rot, rot[:, self.joint_name.index("RootJoint")]
                    )
                )
            elif feature_name == "hipVel":
                features.append(
                    self.extract_vel(
                        root_rot, vel[:, self.joint_name.index("RootJoint")]
                    )
                )
...
class CharacterController:
    def __init__(
        self, viewer, controller, selected_feature_names, selected_feature_weights
    ):
        self.viewer = viewer
    ...
    def update_state(
        ...
        if (
            np.linalg.norm(self.desired_vel_change) < eps
            and np.linalg.norm(self.controller.desired_velocity_change) > eps
        ):
            should_search = True

        if (
            np.linalg.norm(self.desired_rot_change) < eps
            and np.linalg.norm(self.controller.desired_rotation_change) > eps
        ):
            should_search = True
        ...
        if should_search:
            ...
            if "trajectoryPos2D" in self.db.feature_names:
                ...
                future_pos = np.concatenate(
                    [np.array(i).reshape(1, 3) for i in desired_pos_list[1:4]], axis=0
                )
                future_pos = self.global_bone_rot.inv().apply(
                    future_pos - self.global_bone_pos.reshape(1, 3)
                )
                start_idx = self.db.feature_start_idxs[
                    self.db.feature_names.index("trajectoryPos2D")
                ]
                end_idx = (
                    start_idx
                    + self.db.feature_dims[
                        self.db.feature_names.index("trajectoryPos2D")
                    ]
                )
            ...
            if "trajectoryRot2D" in self.db.feature_names:
                # If you change the 20, 40, 60 setting in the Database class, you should change it here as well.
                # For example, if you change it to 20, 40, 60, 80, you should change the index here to 1:5
                future_rot = np.concatenate(
                    [np.array(i).reshape(1, 4) for i in desired_rot_list[1:4]], axis=0
                )
                future_rot = (
                    self.global_bone_rot.inv()
                    * R.from_quat(future_rot[:, [1, 2, 3, 0]])
                ).apply(np.array([0, 0, 1]))
                start_idx = self.db.feature_start_idxs[
                    self.db.feature_names.index("trajectoryRot2D")
                ]
                end_idx = (
                    start_idx
                    + self.db.feature_dims[
            ...
            if self.pre_motion_data is not None:
                ...
                target_rot[1] = R.from_quat(desired_rot_list[0]).as_euler(
                    "XYZ", degrees=True
                )
                target_avel = self.db.local_a_velocities[idx].reshape(-1, 3).copy()
                self.rot_offset, self.avel_offset = (
                    InterpolationHelper.inertialize_transition_rot(
                        self.rot_offset,
                        self.avel_offset,
                        self.pre_motion_data,
                        self.pre_avel,
                        target_rot,
                        target_avel,
                    )
                ...
                self.pos_offset, self.vel_offset = (
                    InterpolationHelper.inertialize_transition_pos(
                        self.pos_offset,
                        self.vel_offset,
                        self.pre_motion_data[[0, 2]],
                        self.pre_vel,
                        target_pos,
                        target_vel,
                    )
        ...
        if self.pre_motion_data is not None:
            ...
            rot, self.pre_avel, self.rot_offset, self.avel_offset = (
                InterpolationHelper.inertialize_update_rot(
                    self.rot_offset,
                    self.avel_offset,
                    target_rot,
                    target_avel,
            ...
            target_pos = np.zeros_like(self.pos_offset)
            target_pos[0] = lerp(
                self.global_bone_pos, self.controller.current_desired_position, 0.9999
            )
            target_pos[1] = self.db.motion_data[idx][6:9]
            target_vel = self.db.local_velocities[idx].reshape(-1, 3)[[0, 2]].copy()
            target_vel[0] = self.controller.vel
            target_vel[1] = 0
            pos, self.pre_vel, self.pos_offset, self.vel_offset = (
                InterpolationHelper.inertialize_update_pos(
                    self.pos_offset,
                    self.vel_offset,
                    target_pos,
                    target_vel,
        ...
        joint_translation, joint_orientation = forward_kinematics_with_channel(
            self.db.joint_parent,
            self.db.joint_channel,
            self.db.joint_offset,
            motion_data.reshape(1, -1),
        )
