...
class MetaData:
    ...
    def __init__(
        self, joint_name, joint_parent, joint_initial_position, root_joint, end_joint
    ):
...
def inverse_kinematics(
    ...
    chain_offsets = [np.array([0.0, 0.0, 0.0])] + [
        meta_data.joint_initial_position[path[i + 1]]
        - meta_data.joint_initial_position[path[i]]
        for i in range(len(path) - 1)
    ]
    chain_orientations = [
        R.from_quat(global_joint_orientations[path[i]]) for i in range(len(path) - 1)
    ] + [R.identity()]
    ...
    if method == "ccd":
        ...
        for _ in range(iteration_num):
            for current_idx in range(end_idx - 1, 0, -1):
                ...
                vec_cur2end = norm(
                    chain_positions[end_idx] - chain_positions[current_idx]
                )
                vec_cur2tar = norm(target_pose - chain_positions[current_idx])

                rot = np.arccos(np.vdot(vec_cur2end, vec_cur2tar))

                if np.isnan(rot) or rot == 0:  # Skip if rotation is not valid
                    continue
                ...
                chain_orientations[current_idx] = (
                    rot_vec * chain_orientations[current_idx]
                )

                ########## Code End ############

                chain_local_rotations = [chain_orientations[0]] + [
                    chain_orientations[i].inv() * chain_orientations[i + 1]
                    for i in range(len(path) - 1)
                ]
                for j in range(current_idx, end_idx):
                    ...
                    if j + 1 < end_idx:
                        chain_orientations[j + 1] = (
                            chain_orientations[j] * chain_local_rotations[j + 1]
                        )
                    else:
    ...
    for joint_idx, parent_idx in enumerate(meta_data.joint_parent):
        if parent_idx == -1:
            ...
            local_joint_rotations[joint_idx] = R.from_quat(
                global_joint_orientations[parent_idx]
            ).inv() * R.from_quat(global_joint_orientations[joint_idx])
    ...
    for chain_idx, joint_idx in enumerate(path2[:-1]):
        global_joint_orientations[path2[chain_idx + 1]] = chain_orientations[
            chain_idx
        ].as_quat()
    ...
    for joint_idx, parent_idx in enumerate(meta_data.joint_parent):
        if parent_idx != -1 and meta_data.joint_name[joint_idx] not in path_name:
            ...
            original_offset = (
                meta_data.joint_initial_position[joint_idx]
                - meta_data.joint_initial_position[parent_idx]
            )
            rotated_offset = R.from_quat(parent_orientation).apply(original_offset)
            global_joint_positions[joint_idx] = (
                global_joint_positions[parent_idx] + rotated_offset
            )
            global_joint_orientations[joint_idx] = (
                R.from_quat(global_joint_orientations[parent_idx])
                * local_joint_rotations[joint_idx]
            ).as_quat()
...
def IK_example(viewer, target_pos, start_joint, end_joint):
    ...
    meta_data = MetaData(
        joint_name, joint_parent, joint_initial_position, start_joint, end_joint
    )
